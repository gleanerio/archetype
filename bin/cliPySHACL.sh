...
# nabu-cli 
# A wrapper script for invoking pySHACL with docker
# Put this script in $PATH as `cliPySHACL.sh`
# docker run --rm -i -t --mount type=bind,src=`pwd`,dst=/data pyshacl -s /data/shacl.ttl /data/graph.ttl


PROGNAME="$(basename $0)"
VERSION="v0.0.1"
NBIMAGE="ashleysommer/pyshacl:latest"
    ...
    a)
      ...
      case $OPTARG in
          docker)
                ...
                shift 2  # drop the now unneeded first two cli params
                exec docker run \
                  --interactive --tty --rm \
                  -e MINIO_USE_SSL \
                  -e MINIO_SECRET_KEY \
